...
import { tenants, tenantResources, tenantEvents, type Tenant } from "../db/schema.js";
...
export async function getTenant(id: string): Promise<Tenant | null> {
  ...
  return { ...results[0], resources } as Tenant & { resources: typeof resources };
}
...
export async function provisionTenant(id: string, version?: string): Promise<Tenant> {
  const tenant = await getTenant(id);
  if (!tenant) throw new Error("Tenant not found");

  await updateTenantStatus(id, "provisioning");
  await logEvent(id, "provisioning_started", "Starting tenant provisioning (Swarm mode)");

  try {
    // Create overlay network
    const networkId = await swarmService.createTenantNetwork(tenant);
    await saveResource(id, "network", networkId, `tenant_${tenant.slug}_network`);
    await logEvent(id, "network_created", "Overlay network created");

    // Create PostgreSQL service
    const postgres = await swarmService.createPostgresService(tenant, `tenant_${tenant.slug}_network`);
    await saveResource(id, "postgres", postgres.serviceId, postgres.serviceName);
    ...
    const redis = await swarmService.createRedisService(tenant, `tenant_${tenant.slug}_network`);
    ...
    await logEvent(id, "provisioning_completed", "Tenant provisioning completed");
...
export async function upgradeTenant(id: string, version: string): Promise<Tenant> {
  const tenant = await getTenant(id);
  if (!tenant) throw new Error("Tenant not found");
  ...
  try {
    ...
    await logEvent(id, "service_updating", "Service update initiated (zero-downtime)");
    ...
    await logEvent(id, "upgrade_completed", `Successfully upgraded to version ${version}`);
...
export async function getTenantHealth(id: string): Promise<{
  status: string;
  services: Record<string, { status: string; replicas: { running: number; desired: number } }>;
}> {
  const tenant = await getTenant(id);
  if (!tenant) throw new Error("Tenant not found");

  const services: Record<string, { status: string; replicas: { running: number; desired: number } }> = {};
  ...
  const anyFailed = Object.values(services).some((s) => s.status === "not_found");
...
// Helper functions
async function updateTenantStatus(id: string, status: Tenant["status"]): Promise<void> {
  await db
    .update(tenants)
