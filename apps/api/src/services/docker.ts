...
export async function createStoreContainer(
	...
	if (existing) {
		...
		const hasCorrectLabels =
			existingLabels[`traefik.http.routers.${tenant.slug}-store.rule`] ===
			expectedHostRule;

		if (!hasCorrectLabels) {
			console.log(
				`Container ${containerName} has outdated labels, recreating...`,
			);
			try {
			...
			console.log(
				`Container ${containerName} already exists with correct labels, reusing...`,
			);
			...
			try {
				...
				const isConnected = networkInfo.Containers?.[existing.Id];
				if (!isConnected) {
	...
	const container = await docker.createContainer({
		...
		Labels: {
			...
			...(IS_PRODUCTION && {
				...
				[`traefik.http.routers.${tenant.slug}-store.tls.certresolver`]:
					"letsencrypt",
			...
			[`traefik.http.services.${tenant.slug}-store.loadbalancer.server.port`]:
				"9000",
		},
		HostConfig: {
			NetworkMode: networkName,
...
export async function createClientContainer(
	...
	if (existing) {
		...
		const hasCorrectLabels =
			existingLabels[`traefik.http.routers.${tenant.slug}-client.rule`] ===
			expectedHostRule;
	...
	const container = await docker.createContainer({
		...
		Labels: {
			...
			"traefik.enable": "true",
			"traefik.docker.network": "platform_network",
			[`traefik.http.routers.${tenant.slug}-client.rule`]: `Host(\`${tenant.subdomain}.${DOMAIN}\`)`,
			[`traefik.http.routers.${tenant.slug}-client.entrypoints`]: IS_PRODUCTION
				? "websecure"
				: "web",
			[`traefik.http.routers.${tenant.slug}-client.priority`]: "100",
			...(IS_PRODUCTION && {
				[`traefik.http.routers.${tenant.slug}-client.tls`]: "true",
				[`traefik.http.routers.${tenant.slug}-client.tls.certresolver`]:
					"letsencrypt",
				[`traefik.http.routers.${tenant.slug}-client.middlewares`]:
					"security-headers@file",
			}),
			[`traefik.http.routers.${tenant.slug}-client.service`]: `${tenant.slug}-client`,
			[`traefik.http.services.${tenant.slug}-client.loadbalancer.server.port`]:
				"3002",
		},
		HostConfig: {
			NetworkMode: networkName,
