 ...
 */
async function getExistingService(serviceName: string): Promise<Docker.Service | null> {
 ...
 * Get service status
 */
async function getServiceStatus(serviceName: string): Promise<{ status: string; running: number; desired: number }> {
  try {
    const services = await docker.listServices({
      filters: { name: [serviceName] },
    });
...
export async function createMedusaService(
  tenant: Tenant,
  networkName: string,
  postgresService: ServiceInfo,
  redisService: ServiceInfo,
  imageTag?: string
): Promise<ServiceInfo> {
 ...
 */
export async function listTenantServices(tenantId: string): Promise<Docker.Service[]> {
