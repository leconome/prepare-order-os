...
clientImagesRoutes.post("/", zValidator("json", createImageSchema), async (c) => {
  try {
    ...
    return c.json(
      { error: error instanceof Error ? error.message : "Failed to create client image" },
      500
    );
  }
});
...
clientImagesRoutes.patch("/:version", zValidator("json", updateImageSchema), async (c) => {
  try {
    ...
    return c.json(
      { error: error instanceof Error ? error.message : "Failed to update client image" },
      500
    );
  }
});
...
clientImagesRoutes.delete("/:version", async (c) => {
  try {
    ...
    return c.json(
      { error: error instanceof Error ? error.message : "Failed to delete client image" },
      500
    );
  }
});
