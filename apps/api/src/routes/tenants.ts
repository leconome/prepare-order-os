...
tenants.post(
	"/:id/provision",
	zValidator("json", provisionTenantSchema.optional()),
	async (c) => {
		try {
			const id = c.req.param("id");
...
tenants.post("/:id/start", async (c) => {
	try {
		...
		return c.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to start tenant",
			},
			500,
		);
	}
...
tenants.post("/:id/restart", async (c) => {
	try {
		...
		return c.json(
			{
				error:
					error instanceof Error ? error.message : "Failed to restart tenant",
			},
			500,
		);
	}
...
tenants.post(
	"/:id/upgrade-client",
	zValidator("json", upgradeClientSchema),
	async (c) => {
		try {
			const id = c.req.param("id");
			const { version } = c.req.valid("json");
