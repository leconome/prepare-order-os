...
export function TenantActions({ tenant, onUpdate, size = "default" }: TenantActionsProps) {
  ...
  return (
    <div className="flex items-center gap-2">
      {tenant.status === "pending" && (
        <Button
          size={buttonSize}
          onClick={() => handleAction("provision", () => api.provisionTenant(tenant.id))}
      ...
      {tenant.status === "stopped" && (
        <Button
          size={buttonSize}
          variant="outline"
          onClick={() => handleAction("start", () => api.startTenant(tenant.id))}
      ...
      {tenant.status === "running" && (
        <>
          <Button
            size={buttonSize}
            variant="outline"
            onClick={() => handleAction("stop", () => api.stopTenant(tenant.id))}
          ...
          <Button
            size={buttonSize}
            variant="outline"
            onClick={() => handleAction("restart", () => api.restartTenant(tenant.id))}
