...
export const createPosDraftOrderWorkflow = createWorkflow(
	...
	(input: CreatePosDraftOrderInput) => {
		...
		const orderInput = transform(
			...
			({ input, storeAddress, itemsWithDetails }) => {
				...
				const shippingAddress = {
					...
					address_1:
						input.shipping_address?.address_1 || storeAddress.address_1,
					city: input.shipping_address?.city || storeAddress.city,
					postal_code:
						input.shipping_address?.postal_code || storeAddress.postal_code,
					country_code:
						input.shipping_address?.country_code || storeAddress.country_code,
		...
		const posOrderInput = transform({ order, input }, ({ order, input }) => ({
			order_id: order.id,
