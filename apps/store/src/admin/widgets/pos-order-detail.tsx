...
const PosOrderDetailWidget = ({
	...
	const { data: posOrderData, isLoading } = useQuery({
		queryFn: async () => {
			...
			const posOrder = response.pos_orders.find(
				(po) => po.order_id === order.id,
			);
	...
	const createPosOrder = useMutation({
		mutationFn: async (
			data: Omit<typeof createFormData, "assigned_to_id"> & {
				order_id: string;
	...
	const handleEditSubmit = () => {
		...
		if (
			formData.assigned_to_id !== (posOrderData?.assigned_to?.id || "_none")
		) {
			updateData.assigned_to_id =
				formData.assigned_to_id === "_none" ? null : formData.assigned_to_id;
		}
	...
	const handleCreateSubmit = () => {
		...
		const assignedId =
			createFormData.assigned_to_id === "_none" ||
